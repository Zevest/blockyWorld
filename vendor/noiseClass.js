...
class Noise {
	...
	gradP[i] = gradP[i + 256] = this.grad3[perm[i] % 12];
	}*/
	...
	simplex2(xin, yin) {
		...
		var s = (xin + yin) * Noise.F2; // Hairy factor for 2D
		...
		// c = (3-sqrt(3))/6
		var x1 = x0 - i1 + Noise.G2; // Offsets for middle corner in (x,y) unskewed coords
		var y1 = y0 - j1 + Noise.G2;
		var x2 = x0 - 1 + 2 * Noise.G2; // Offsets for last corner in (x,y) unskewed coords
		var y2 = y0 - 1 + 2 * Noise.G2;
	...
	// 3D simplex noise
		...
		// Skew the input space to determine which simplex cell we're in
		var s = (xin + yin + zin) * Noise.F3; // Hairy factor for 2D
		var i = Math.floor(xin + s);
		var j = Math.floor(yin + s);
		...
		j &= 255;
		k &= 255;
		var gi0 = Noise.gradP[i + this.perm[j + this.perm[k]]];
		var gi1 = Noise.gradP[i + i1 + this.perm[j + j1 + this.perm[k + k1]]];
		var gi2 = Noise.gradP[i + i2 + this.perm[j + j2 + this.perm[k + k2]]];
		var gi3 = Noise.gradP[i + 1 + this.perm[j + 1 + this.perm[k + 1]]];
