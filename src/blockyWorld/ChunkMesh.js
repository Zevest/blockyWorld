class ChunkMesh {
    ...
    static rebuild(chunkMeshs, chunkData) {
    ...
    static deleteData(object3DChunk) {
        let chunkMesh = object3DChunk.userData.chunkMeshData;
        chunkMesh.geometry.block.opaque.dispose();
        chunkMesh.geometry.block.semi.dispose();
        chunkMesh.geometry.block.transparent.dispose();
        chunkMesh.geometry.cross.dispose();

        for(let mesh of object3DChunk.children){
            object3DChunk.remove(mesh);
        }
        /*chunkMesh.mesh.block.opaque.dispose();
        chunkMesh.mesh.block.semi.dispose();
        chunkMesh.mesh.block.transparent.dispose();
        chunkMesh.mesh.cross.dispose();*/
    }
}